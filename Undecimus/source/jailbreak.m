...
#undef cache_address
    ...
    
    {
        ...
        if ((needStrap || !pkgIsConfigured("firmware")) ||
            ...
            if (pkgIsConfigured("cydia") && compareInstalledVersion("cydia", "lt", "1:0")) {
                NSArray <NSString *> *const fwDebs = debsForPkgs(@[@"cydia", @"cydia-lproj"]);
            ...
            if (pkgIsConfigured("uikittools") && compareInstalledVersion("uikittools", "lt", "2.0.0")) {
                _assert(installDeb("uikittools", true), localize(@"Unable to install uikittools."), true);
            }
            if (access("/usr/libexec/cydia/firmware.sh", F_OK) != ERR_SUCCESS || !pkgIsConfigured("cydia")) {
                LOG("Extracting Cydia...");
                NSArray <NSString *> *const fwDebs = debsForPkgs(@[@"cydia", @"darwintools", @"uikittools", @"system-cmds"]);
                _assert(fwDebs != nil, localize(@"Unable to get firmware debs."), true);
                _assert(installDebs(fwDebs, true, false), localize(@"Unable to install firmware debs."), true);
            }
            rv = _system("/usr/libexec/cydia/firmware.sh");
        ...
        if ((pkgIsInstalled("apt") && pkgIsBy("Bingner", "apt")) ||
            ...
            NSArray <NSString *> *const aptdebs = debsForPkgs(@[@"apt-lib", @"apt-key", @"apt"]);
            _assert(aptdebs != nil && aptdebs.count == 3, localize(@"Unable to get debs for apt."), true);
            for (id deb in aptdebs) {
        ...
        if(pkgIsInstalled("com.diatrus.sileo-installer")) {
            ...
            _assert(removePkg("com.diatrus.sileo-installer", true), localize(@"Unable to remove old Sileo Installer."), true);
            LOG("Removed Diatrus's Sileo Installer.");
        }
        if(pkgIsInstalled("us.diatr.sillyo2")) {
            LOG("Removing Diatrus's Sileo Compatibility Layer...");
            _assert(removePkg("us.diatr.sillyo2", true), localize(@"Unable to remove old Sileo Compatibility Layer."), true);
            LOG("Removed Diatrus's Sileo Compatibility Layer.");
        }
        if(pkgIsInstalled("org.juulstar.sileo")) {
            LOG("Removing Old Sileo...");
            _assert(removePkg("org.juulstar.sileo", true), localize(@"Unable to remove old Sileo."), true);
        ...
        NSString *const listContents = @"deb [trusted=yes] file:///var/lib/undecimus/apt ./\n";
        ...
        if (!pkgIsConfigured("apt") || !aptUpdate()) {
            NSArray *const aptNeeded = resolveDepsForPkg(@"apt", false);
    ...
    
    {
        ...
        if (prefs->install_cydia) {
            ...
            NSString *const cydiaVer = versionOfPkg(@"cydia-dark");
            _assert(cydiaVer != nil, localize(@"Unable to get Cydia version."), true);
            _assert(aptInstall(@[@"--reinstall", [@"cydia-dark" stringByAppendingFormat:@"=%@", cydiaVer]]), localize(@"Unable to reinstall Cydia."), true);
        ...
        if (prefs->install_sileo) {
            progress(localize(@"Installing Sileo..."));
            NSString *const sileoVer = versionOfPkg(@"org.coolstar.sileo");
            NSString *const compatVer = versionOfPkg(@"us.diatr.sillyo");
            _assert(sileoVer != nil, localize(@"Unable to get Sileo version."), true);
            _assert(compatVer != nil, localize(@"Unable to get Sileo Compatibility Layer version."), true);
            _assert(aptInstall(@[@"--reinstall", [@"org.coolstar.sileo" stringByAppendingFormat:@"=%@", sileoVer]]), localize(@"Unable to reinstall Sileo."), true);
            _assert(aptInstall(@[@"--reinstall", [@"us.diatr.sillyo" stringByAppendingFormat:@"=%@", compatVer]]), localize(@"Unable to reinstall Sileo Compatibility Layer."), true);
            prefs->install_sileo = false;
            prefs->uicache_sileo = true;
            sync_prefs();
            LOG("Successfully installed Sileo.");
            
            insertstatus(localize(@"Installed Sileo.\n"));
        }
    }
    ...
    
    {
        if (prefs->run_uicache) {
            ...
            _assert(runCommand("/usr/bin/uicache", "--all", NULL) == ERR_SUCCESS, localize(@"Unable to refresh icon cache."), true);
